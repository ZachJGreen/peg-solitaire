...
class TestAC5_3:
    def test_board_locked_after_game_over(self):
        ...
        game.handle_click(3, 4)
        assert game.board.grid == final_grid, "Board grid should not change after game over"
        assert game.moves_made == final_moves, "Move count should not change after game over"
        assert game.selected is None, "No selection should be allowed after game over"
